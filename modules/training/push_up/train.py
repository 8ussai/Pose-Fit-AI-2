...
class Trainer:
    def __init__(self):
        ...
        self.processed_dir = config.PUSH_UP_PROCESSED_DIR
        self.models_dir = config.MODELS_DIR / "push_up"
        self.models_dir.mkdir(parents=True, exist_ok=True)

        full_train = PushUpPTDataset(self.processed_dir, split="train")
        ...
        full_val = PushUpPTDataset(self.processed_dir, split="val")
    ...
    def plot_curves(self):
        out_dir = config.MODELS_DIR / "push_up"
        ...
        plt.legend()
        plt.title("Push-Up - Accuracy Curves")
        plt.savefig(out_png2, bbox_inches="tight")
        plt.close()

    def fit(self):
        best_path = self.models_dir / "push_up_twostream_mobilenet_GRU_model.pt"
