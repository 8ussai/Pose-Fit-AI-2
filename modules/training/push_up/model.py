...
class TwoStreamMobileNetGRU(nn.Module):
    def __init__(self, num_classes=3, hidden=256, layers=1, dropout=0.3, freeze_backbone=True):
        super().__init__()
    ...
    def forward(self, original_frames, mediapipe_frames):
        ...
        x1 = original_frames.reshape(B * T, C, H, W)
        x2 = mediapipe_frames.reshape(B * T, C, H, W)

        f1 = self.spatial(x1)
        f1 = self.gap(f1).flatten(1)

        f2 = self.pose(x2)
        f2 = self.gap(f2).flatten(1)

        fused = torch.cat([f1, f2], dim=1)
        fused = fused.reshape(B, T, -1)

        out, _ = self.temporal(fused)
        last = out[:, -1, :]
